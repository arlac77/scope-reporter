...
const rootScopeDefinition = {
  toString() {
    ...
    valueString() {
      ...
      return Object.keys(values).map(function(key) { return values[key]; } ).join(' ');
    },
...
{
  if(aConsole === undefined) { aConsole = console; }

  return function(reporter) {
    const message = reporter.scopeStack.map( function(scope) { return `${scope.name}: ${scope.valueString()}`; }).join(',');
...
exports.createReporter = function (scopeDefinitionsRaw, reportAdapter) {
  ...
  const root = {
    toString() {
      ...
      exception(e) {
        this.reportWithSeverity('fatal', undefined, 'exception', { exception: e});
      }
  };
